...
interface SignInScreenPropsWithOnboarding extends SignInScreenProps {
  onAuthComplete?: (() => void) | undefined;
  onSkipAuth?: (() => void) | undefined;
}

export const SignInScreen: React.FC<SignInScreenPropsWithOnboarding> = ({
  onAuthComplete,
  onSkipAuth,
}) => {
  ...
  const handleBack = useCallback(() => {
    if (onSkipAuth) {
      // In onboarding context, call skip handler
      onSkipAuth();
    } else {
      // In main app context, go back
      navigation.getParent()?.goBack();
    }
  }, [navigation, onSkipAuth]);
  ...
  const handleForgotPassword = useCallback(
    (email?: string, phone?: string, method?: 'email' | 'phone') => {
