      ...
      {values.map((v, idx) => (
        <div key={v.stage} className='flex items-center gap-3'>
          <div className='flex flex-col items-center'>
            <div
              className={`h-7 w-7 rounded-full flex items-center justify-center text-white text-xs font-semibold ${v.status === 'completed' ? 'bg-accent-600' : v.status === 'in-progress' ? 'bg-accent-600/70' : 'bg-neutral-400'}`}
            >
              {v.status === 'completed'
                ? '✓'
...
export const StageStatsRow: React.FC<{
    ...
    <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
      ...
      <div className='rounded-xl border border-neutral-200 dark:border-neutral-800 p-3 text-center'>
        ...
        <div className='text-2xl font-bold'>
          {distributionCount.toLocaleString()}
        </div>
