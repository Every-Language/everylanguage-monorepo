...
export function useRegionBoundary(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const row = Array.isArray(data)
        ? (data[0] as { boundary?: unknown } | undefined)
        : (data as { boundary?: unknown } | null);
      const boundary =
        row && 'boundary' in row
          ? (row as { boundary?: unknown }).boundary
          : null;
...
export function useLanguageOverlayGeometries(
  languageEntityId: string | null | undefined,
  opts?: { enabled?: boolean }
) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const features = (data ?? [])
        .map(
          (r: { regions: { boundary_simplified: unknown } }) =>
            (r.regions?.boundary_simplified ?? null) as GeoJSON.Geometry | null
        )
...
export function useLanguageOverlayGeometriesForIds(
  languageEntityIds: string[] | null | undefined,
  opts?: { enabled?: boolean }
) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const features = (data ?? [])
        .map(
          (r: { regions: { boundary: unknown } }) =>
            (r.regions?.boundary ?? null) as GeoJSON.Geometry | null
        )
