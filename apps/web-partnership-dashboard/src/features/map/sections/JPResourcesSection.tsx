 ...
 */
export const JPResourcesSection: React.FC<JPResourcesSectionProps> = ({
  type,
  entityId,
}) => {
  ...
  const ResourceLink = ({
    ...
    >
      ...
      <ArrowTopRightOnSquareIcon className='w-4 h-4 text-neutral-400 group-hover:text-secondary-600 dark:group-hover:text-secondary-400' />
    </a>
  );
      ...
      {isRegion && countryData.countryStats && (
        <ResourceCategory title='Maps & Geography' categoryId='maps'>
          <ResourceLink
            ...
            href={`https://joshuaproject.net/countries/${countryData.countryStats.ROG3}`}
          />
          {countryData.countryStats.Longitude &&
            countryData.countryStats.Latitude && (
              <ResourceLink
