...
import { useUserDonations } from '../api/useUserDonations';
...
export const UserDonationsPage: React.FC = () => {
  const router = useRouter();
  const { user } = useAuth();

  const { data: donations, isLoading } = useUserDonations(user?.id ?? null);
      ...
      <div className='space-y-6'>
        <div>
          ...
          <p className='text-sm text-neutral-500 dark:text-neutral-400 mt-1'>
            View all donations made to Every Language
          </p>
        </div>
        <TableRowSkeleton count={7} columns={5} />
      </div>
    );
  }
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='space-y-0'>
        {/* Table Header */}
        <div className='grid grid-cols-[1fr_1fr_1fr_1fr_2fr] gap-4 px-4 py-3 border-b border-neutral-200 dark:border-neutral-800 text-xs font-medium text-neutral-500 dark:text-neutral-400'>
          <div>Date</div>
          <div>Partner Org</div>
          <div>Status</div>
          <div>Amount</div>
        ...
        {donations.map(donation => {
          ...
          const partnerOrg = normalizeSupabaseRelation(donation.partner_org);
          const partnerOrgName = partnerOrg
            ? `${partnerOrg.name}${partnerOrg.is_individual ? ' (individual)' : ''}`
            : 'N/A';

          return (
            <div
              key={donation.id}
              className='grid grid-cols-[1fr_1fr_1fr_1fr_2fr] gap-4 px-4 py-4 border-b border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors'>
              {/* Date Column */}
              ...
              <div className='space-y-1'>
                ...
                {donation.is_recurring && donation.subscription && (
                  <div className='flex items-center gap-1'>
                    ...
                    {normalizeSupabaseRelation(donation.subscription)
                      ?.status === 'active' && (
                      <Button
                        variant='ghost'
                        size='xs'
