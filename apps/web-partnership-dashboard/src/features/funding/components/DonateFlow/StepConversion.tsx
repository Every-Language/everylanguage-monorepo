...
export const StepConversion: React.FC<StepConversionProps> = ({
  ...
  const { setCheckoutPromise, coverFees } =
  ...
  const handleConversion = (monthlyAmount: number | null) => {
    if (monthlyAmount) {
      // User chose monthly - update amount to monthly
      flow.setAmount({
        isRecurring: true,
    ...
    const checkoutPromise = createDonationCheckout({
      ...
      donorType: flow.state.donorType?.type ?? 'individual',
      partnerOrgId: flow.state.donorType?.partnerOrgId,
      newPartnerOrg: flow.state.donorType?.newPartnerOrg,
      intent: flow.state.intent ?? { type: 'unrestricted' },
      paymentMethod: flow.state.paymentMethod ?? 'card',
      amountCents: totalCents,
      isRecurring: !!monthlyAmount,
    }).then(response => ({
      ...response,
      partnerOrgId: response.partnerOrgId ?? '',
    }));
