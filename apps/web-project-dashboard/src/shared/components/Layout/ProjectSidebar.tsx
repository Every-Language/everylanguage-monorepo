...
import { useSidebarCollapsed, useToggleSidebar } from '../../stores/sidebar';
import {
...
export const ProjectSidebar: React.FC = () => {
  ...
  const { project, closeProject, isLoading } = useProjectFromRoute();
  ...
  // Extract language IDs from project
  const languageIds = useMemo(() => {
    const ids: string[] = [];
    if (project?.source_language_entity_id)
      ids.push(project.source_language_entity_id);
    if (project?.target_language_entity_id)
      ids.push(project.target_language_entity_id);
    return ids;
  }, [project]);
    ...
    <>
      ...
      {isCollapsed && (
        <button
          ...
          className='fixed left-4 top-4 z-50 p-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-md hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors'
          title='Open sidebar'>
