...
export function FuzzySearchSelector({
  ...
  const languageSearch = useLanguagesProgressiveSearch(
    debouncedQuery,
    searchType === 'language' && debouncedQuery.length >= 2 && shouldSearch
  );
  const regionSearch = useRegionsProgressiveSearch(
    debouncedQuery,
    searchType === 'region' && debouncedQuery.length >= 2 && shouldSearch
  );

  const searchResults =
    searchType === 'language' ? languageSearch : regionSearch;

  // Transform results to common format
  const transformedResults: SearchResultItem[] =
    searchResults.data?.results?.map(
      (result: LanguageSearchResult | RegionSearchResult) => ({
        id:
          searchType === 'language'
            ? (result as LanguageSearchResult).entity_id
            : (result as RegionSearchResult).region_id,
        name:
          searchType === 'language'
            ? (result as LanguageSearchResult).entity_name
            : (result as RegionSearchResult).region_name,
        level:
          searchType === 'language'
            ? (result as LanguageSearchResult).entity_level
            : (result as RegionSearchResult).region_level,
              ...
              <div className='px-3 py-2 bg-neutral-50 dark:bg-neutral-800 border-t border-neutral-200 dark:border-neutral-700'>
                <div className='text-xs text-neutral-500 dark:text-neutral-400 flex items-center justify-between'>
                  ...
                  <span>
                    ...
                    {Math.round(
                      searchResults.data.metadata.thresholdUsed * 100
                    )}
                    %
                  </span>
                </div>
