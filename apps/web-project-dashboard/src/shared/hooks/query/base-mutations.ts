...
export function useCreateRecord<T extends TableName>(
  ...
  options?: {
    ...
    UseMutationOptions<TableRow<T>, SupabaseError, TableInsert<T>>,
    'mutationFn'
  >
): UseMutationResult<TableRow<T>, SupabaseError, TableInsert<T>> {
  const queryClient = useQueryClient();
  const { onSuccess, onError, invalidateQueries, ...mutationOptions } =
    options || {};
...
export function useUpdateRecord<T extends TableName>(
  ...
  options?: {
    ...
    UseMutationOptions<
      TableRow<T>,
      SupabaseError,
      { id: string; updates: TableUpdate<T> }
    >,
    'mutationFn'
  >
): UseMutationResult<
    ...
    onSuccess: data => {
      ...
      if (invalidateQueries) {
        ...
        queryClient.invalidateQueries({
          queryKey: [table, 'detail', (data as RecordWithId).id],
        });
      }
