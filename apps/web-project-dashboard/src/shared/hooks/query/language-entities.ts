...
export function useLanguagesFuzzySearch(
  ...
  return useQuery<{
    ...
    queryFn: async () => {
      ...
      const { data, error } = await (supabase as SupabaseWithRPC).rpc(
...
export function useLanguagesProgressiveSearch(
  query: string,
  ...
  return useQuery<{
    ...
    queryFn: async () => {
      ...
      const { data: initialData, error: initialError } = await (
        supabase as SupabaseWithRPC
      ).rpc('search_language_aliases', {
        search_query: query,
        max_results: 15,
        min_similarity: 0.35,
      ...
      if (!data || data.length < 5) {
        const { data: lenientData, error: lenientError } = await (
          supabase as SupabaseWithRPC
        ).rpc('search_language_aliases', {
          search_query: query,
          max_results: 25,
      ...
      if (!data || data.length < 3) {
        const { data: veryLenientData, error: veryLenientError } = await (
          supabase as SupabaseWithRPC
        ).rpc('search_language_aliases', {
          search_query: query,
          max_results: 35,
...
export function useLanguageHierarchy(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: hierarchyData, error: hierarchyError } = await (
...
export function useLanguagesWithRegions(
  languageIds: string[],
  enabled: boolean = true
) {
  return useQuery({
