...
export function useCreateProjectOptimistic() {
  ...
  const { createOptimistic, addToCollection, rollback, replaceOptimisticData } =
    useOptimisticUpdates();
  const { showOptimisticFeedback, showSuccessFeedback, showErrorFeedback } =
    useOptimisticMutationState();
  ...
  return useMutation({
    ...
    onSuccess: (data, _variables, context) => {
      if (context?.optimisticProject) {
        ...
        replaceOptimisticData(['projects'], context.optimisticProject.id, data);
      }
...
export function useUpdateProjectOptimistic() {
  const queryClient = useQueryClient();
  const { updateOptimistic, updateInCollection, rollback } =
    useOptimisticUpdates();
  const { showOptimisticFeedback, showSuccessFeedback, showErrorFeedback } =
    useOptimisticMutationState();
  ...
  return useMutation({
    ...
    onMutate: async ({ id, updates }) => {
      ...
      if (currentProject) {
        // Create optimistic data
        const optimisticProject = updateOptimistic(
          'projects',
          id,
          updates,
          currentProject
        ...
        const collectionContext = updateInCollection(
          ['projects'],
          id,
          optimisticProject
        );
...
export function useDeleteProjectOptimistic() {
  const queryClient = useQueryClient();
  const { removeFromCollection, rollback } = useOptimisticUpdates();
  const { showOptimisticFeedback, showSuccessFeedback, showErrorFeedback } =
    useOptimisticMutationState();
