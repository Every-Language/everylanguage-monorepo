...
export class AudioFileProcessor {
  async processFile(
    ...
    if (filenameParseResult.isFullChapter && bibleVersionId) {
      try {
        filenameParseResult = await resolveFullChapterEndVerse(
          filenameParseResult,
          bibleVersionId
        );
      } catch (error) {
        console.warn(
          'Failed to resolve full chapter end verse for',
          file.name,
          error
        );
      }
    ...
    // Validate
    const validationErrors = this.validateFile(
      file,
      filenameParseResult,
      audioMetadata
    );
  ...
  private validateFile(
    ...
    if (
      ...
      errors.push(
        'Unsupported file type. Please use MP3, WAV, M4A, AAC, OGG, or WebM files'
      );
    }
    ...
    if (parsed.confidence === 'none') {
      errors.push(
        'Could not auto-detect book/chapter from filename - manual selection required'
      );
    }
   ...
   */
  private async processFileWithParsedResult(
    file: File,
    filenameParseResult: ParsedFilename
  ): Promise<ProcessedAudioFile> {
    const id = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  updateProcessedFile(
    ...
    updates: Partial<ProcessedAudioFile>
  ): ProcessedAudioFile {
    const updatedFile: ProcessedAudioFile = { ...file, ...updates };

    // Re-validate if validation-related fields changed
    if (
      updates.selectedBookId ||
      updates.selectedChapterId ||
      updates.selectedStartVerseId ||
      updates.selectedEndVerseId
    ) {
      const hasAllRequired = !!(
        updatedFile.selectedBookId &&
        updatedFile.selectedChapterId &&
        updatedFile.selectedStartVerseId &&
        updatedFile.selectedEndVerseId
      );

      if (
        hasAllRequired &&
        updatedFile.validationErrors.includes(
          'Could not auto-detect book/chapter from filename - manual selection required'
        )
      ) {
        updatedFile.validationErrors = updatedFile.validationErrors.filter(
          error => !error.includes('manual selection required')
        );
