  ...
  startUpload: async (files, projectData, userId, queryClient) => {
    ...
    try {
      ...
      const byId = functionResponse as {
        success: boolean;
        images?: Array<{ id: string; objectKey: string; uploadUrl: string }>;
      ...
      const worker = async () => {
        while (idx < files.length) {
          ...
          try {
            ...
            await imageService.finalizeImage({
              imageId: id,
              fileSize: file.file.size,
            });
