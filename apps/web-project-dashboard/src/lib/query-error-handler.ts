...
export function processQueryError(error: unknown): ProcessedError {
  ...
  if (error && typeof error === 'object' && 'message' in error) {
    ...
    const errorType = supabaseError.code
      ? errorCodeMap[supabaseError.code] || ErrorType.UNKNOWN
      : ErrorType.UNKNOWN;
...
function isRetryable(errorType: ErrorType): boolean {
  return [ErrorType.NETWORK, ErrorType.SERVER].includes(errorType);
}

// Check if an error type should be reported
function isReportable(errorType: ErrorType): boolean {
  return [ErrorType.SERVER, ErrorType.UNKNOWN].includes(errorType);
}
