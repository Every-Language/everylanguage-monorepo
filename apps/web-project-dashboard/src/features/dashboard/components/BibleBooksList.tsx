...
export const BibleBooksList: React.FC<BibleBooksListProps> = ({
  ...
  const {
    ...
    isLoading,
    error,
  } = useBibleProjectDashboard(projectId);
  ...
  const statusLegend = [
    {
      status: 'complete',
      label: 'Complete',
      description: 'All verses recorded',
    },
    {
      status: 'in_progress',
      label: 'In Progress',
      description: 'Some verses recorded',
    },
    {
      status: 'not_started',
      label: 'Not Started',
      description: 'No verses recorded',
    },
  ] as const;
  ...
  const filteredBooks = useMemo(() => {
    ...
    return bibleData.books
      .filter(book => {
        ...
        let bookStatus: 'complete' | 'in_progress' | 'not_started' =
          'not_started';
      ...
      .map(book => {
        ...
        return {
          ...
          chapters: book.chapters.filter(
            chapter => chapter.status === chapterStatusFilter
          ),
        };
      });
    ...
    return (
      <div className='flex items-center justify-center h-64'>
        <div className='text-red-600 dark:text-red-400'>
          Error loading Bible books: {error.message}
        </div>
      </div>
    );
  }
  ...
  if (!bibleData?.books || bibleData.books.length === 0) {
    return (
      <div className='flex items-center justify-center h-64'>
        <div className='text-neutral-600 dark:text-neutral-400'>
          No Bible books found for this project.
        </div>
      </div>
    );
  }
