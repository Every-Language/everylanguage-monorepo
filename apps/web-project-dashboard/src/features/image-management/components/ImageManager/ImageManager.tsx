...
export const ImageManager: React.FC = () => {
  ...
  if (!imageManagement.user) {
    return <Alert variant='warning'>Please log in to view your images.</Alert>;
  }
  ...
  const imageSetSelector = (
    <VersionSelector
      ...
      onVersionChange={setId =>
        imageManagement.operations.handleFilterChange('setId', setId)
      }
      versions={imageManagement.imageSets.map(set => ({
        id: set.id,
        name: set.name,
      }))}
  ...
  const tableSection = (
    <ImageTable
      ...
      someCurrentPageSelected={
        imageManagement.selection.someCurrentPageSelected
      }
      ...
      onPublishStatusChange={
        imageManagement.operations.handlePublishStatusChange
      }
      ...
      updatePublishStatusPending={
        imageManagement.mutations.updatePublishStatus.isPending
      }
      // Search functionality
      searchText={imageManagement.filters.searchText}
      onSearchChange={value =>
        imageManagement.operations.handleFilterChange('searchText', value)
      }
      // Bulk operations
      onBulkPublishStatusChange={
        imageManagement.operations.handleBulkPublishStatusChange
      }
      onClearSelection={imageManagement.selection.clearSelection}
      batchUpdatePending={
        imageManagement.mutations.batchUpdatePublishStatus.isPending
      }
    />
  );
    ...
    <>
      ...
      {imageManagement.modals.showEdit && imageManagement.editingImage && (
        <EditImageModal
          ...
          isPending={
            imageManagement.mutations.updateImage.isPending ||
            imageManagement.mutations.updatePublishStatus.isPending
          }
        />
      )}
