  ...
  const versionSelector = (
    <VersionSelector
      ...
      onVersionChange={versionId =>
        audioFileState.handleFilterChange('audioVersionId', versionId)
      }
  ...
  const table = (
    <AudioFileTable
      ...
      onSearchChange={value =>
        audioFileState.handleFilterChange('searchText', value)
      }
  ...
  const modals = (
    <>
      <AudioUploadModal
        ...
        onOpenChange={open =>
          open
            ? audioFileState.openModal('upload')
            : audioFileState.closeModal()
        }
        onUploadComplete={audioFileState.handleUploadComplete}
        selectedAudioVersionId={
          audioFileState.filters.audioVersionId !== 'all'
            ? audioFileState.filters.audioVersionId
            : ''
        }
      />
      ...
      <AudioVersionModal
        ...
        onOpenChange={open =>
          open
            ? audioFileState.openModal('audioVersion')
            : audioFileState.closeModal()
        }
      ...
      <VerseTimestampImportModal
        ...
        onOpenChange={open =>
          open
            ? audioFileState.openModal('verseTimestampImport')
            : audioFileState.closeModal()
        }
      ...
      <ConfirmationModal
        ...
        title={
          audioFileState.confirmationModal.type === 'delete'
            ? 'Delete Files'
            : 'Restore Files'
        }
        ...
        variant={
          audioFileState.confirmationModal.type === 'delete' ? 'danger' : 'info'
        }
        isLoading={
          audioFileState.softDeleteFiles.isPending ||
          audioFileState.restoreFiles.isPending
        }
      />
    </>
  );
          ...
          <CardContent className='pt-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <span className='text-sm text-gray-500'>
                {Math.round(audioFileState.downloadState.progress)}%
              </span>
            </div>
            <Progress
              value={audioFileState.downloadState.progress}
              className='w-full'
            />
          </CardContent>
        </Card>
