...
export const VerseMarkingModal: React.FC<VerseMarkingModalProps> = ({
  ...
  const verseConstraints = React.useMemo(() => {
    if (!mediaFile)
      return { minVerseNumber: 1, maxVerseNumber: 1, maxVerses: 1 };
    ...
    if (open && existingVerses.length > 0) {
      const converted = existingVerses
        .map((verse, index) => ({
          ...
          verse_number:
            verse.verse_number || verseConstraints.minVerseNumber + index,
        }))
  ...
  const validateVerseDuration = useCallback(
    (newTimestamp: number, verseId?: string) => {
      ...
      const sortedVerses = [...otherVerses, { timestamp: newTimestamp }].sort(
        (a, b) => a.timestamp - b.timestamp
      );
      const currentIndex = sortedVerses.findIndex(
        v => v.timestamp === newTimestamp
      );
  ...
  const addVerseAtCurrentTime = useCallback(() => {
    ...
    if (tempVerses.length >= verseConstraints.maxVerses) {
      setError(
        `Cannot add more verses. This audio file can only have ${verseConstraints.maxVerses} verse(s) (${mediaFile?.verse_reference}).`
      );
      return;
    }
    ...
    tempVerses,
    verseConstraints,
    mediaFile?.verse_reference,
    validateVerseDuration,
  ]);
  ...
  const updateVerseTimestamp = useCallback(
    (verseId: string, newTimestamp: number) => {
      // Validate verse duration constraint
      ...
      const newVerses = tempVerses
        .map(verse =>
          verse.id === verseId
            ? { ...verse, timestamp: parseFloat(newTimestamp.toFixed(2)) }
            : verse
        )
        .sort((a, b) => a.timestamp - b.timestamp)
        .map((verse, index) => ({
  ...
  const getCurrentVerse = () => {
    return tempVerses.find((verse, index) => {
      ...
      return (
        currentTime >= verse.timestamp &&
        (!nextVerse || currentTime < nextVerse.timestamp)
      );
    });
  };
          ...
          {audioUrl && !isLoadingAudio && (
            <div className='flex justify-center flex-shrink-0'>
              ...
              >
                ...
                <span>
                  Mark Verse{' '}
                  {verseConstraints.minVerseNumber + tempVerses.length} at{' '}
            ...
            <div className='flex-1 overflow-hidden flex flex-col'>
              <div className='flex items-center justify-between mb-3 flex-shrink-0'>
                <h3 className='text-base font-medium text-primary-600 dark:text-primary-400'>
                  Marked Verses ({tempVerses.length}/
                  {verseConstraints.maxVerses})
                </h3>
                {currentVerse && (
              ...
              <div className='flex-1 overflow-y-auto space-y-2 min-h-0'>
                {tempVerses.length === 0 ? (
                  <div className='text-center py-8 text-neutral-500 dark:text-neutral-400'>
                    <p>
                      No verses marked. Click "Mark Verse" to add verses at
                      specific timestamps.
                    </p>
                  </div>
                ) : (
