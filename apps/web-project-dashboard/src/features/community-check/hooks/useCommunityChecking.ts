...
export function useCommunityChecking() {
  ...
  const pendingCheckFiles = useMemo(
    () =>
      allMediaFiles?.filter(
        file =>
          file.check_status === 'pending' && file.upload_status === 'completed'
      ) || [],
    [allMediaFiles]
  );
  ...
  const handlePublishFile = useCallback(
    async (fileId: string) => {
      try {
  ...
  const handleUpdateFileStatus = useCallback(
    async (fileId: string, newStatus: CheckStatus) => {
      try {
        await updateCheckStatusMutation.mutateAsync({
          mediaFileId: fileId,
          newStatus,
