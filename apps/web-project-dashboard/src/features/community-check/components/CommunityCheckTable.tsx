...
export function CommunityCheckTable({
  onStartChecking,
  selectedFileId,
  compact = false,
  ...
  // Fetch data
  const { data: audioVersions = [], isLoading: audioVersionsLoading } =
    useAudioVersionsByProject(selectedProject?.id || '');
  ...
  const {
    ...
    isLoading,
    error,
  } = useMediaFilesByProject(selectedProject?.id || null);
  ...
  const filteredFiles =
    allMediaFiles?.filter(file => {
      const matchesCheckStatus =
        selectedCheckStatus === 'all' ||
        file.check_status === selectedCheckStatus;
      const matchesUploadStatus = file.upload_status === 'completed';
      const matchesSearch =
        !searchText ||
        file.verse_reference?.toLowerCase().includes(searchText.toLowerCase());
      const matchesAudioVersion =
        selectedAudioVersion === 'all' ||
        file.audio_version_id === selectedAudioVersion;
      const matchesBook =
        selectedBook === 'all' || file.book_id === selectedBook;
      const matchesChapter =
        selectedChapter === 'all' || file.chapter_id === selectedChapter;
