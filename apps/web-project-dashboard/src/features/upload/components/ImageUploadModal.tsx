...
export function ImageUploadModal({
  ...
  const [setSelectionMode, setSetSelectionMode] = useState<'existing' | 'new'>(
    'existing'
  );
  ...
  const updateFile = useCallback(
    (fileId: string, updates: Partial<ProcessedImageFile>) => {
      setImageFiles(prev =>
  ...
  const handleUpload = useCallback(async () => {
    ...
    const validFiles = imageFiles.filter(f => f.isValid && f.selectedTargetId);
  ...
  const filesReadyForUpload = imageFiles.filter(
    f => f.isValid && f.selectedTargetId
  ).length;
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className='max-w-4xl max-h-[90vh] overflow-hidden flex flex-col'>
          <DialogHeader>
            ...
            <DialogDescription>
              Upload images and organize them into sets. Supported formats:
                ...
                <div className='flex space-x-4'>
                  <label className='flex items-center'>
                    <input
                      ...
                      onChange={e =>
                        setSetSelectionMode(e.target.value as 'existing')
                      }
                      className='mr-2'
                    />
                    Use existing set
                  ...
                  <label className='flex items-center'>
                    <input
                      ...
                      onChange={e =>
                        setSetSelectionMode(e.target.value as 'new')
                      }
                      className='mr-2'
                    />
                    Create new set
              ...
              <div className='space-y-4'>
                ...
                <div className='bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg'>
                  <p className='text-sm text-blue-800 dark:text-blue-200'>
                    {filesReadyForUpload} of {imageFiles.length} files ready for
                    upload
                  </p>
                </div>
              </div>
            ...
            >
              {isUploading ? (
                ...
                <>
                  ...
                  Upload {filesReadyForUpload} Image
                  {filesReadyForUpload !== 1 ? 's' : ''}
                </>
              )}
            </Button>
          </DialogFooter>
