...
export const BaseDashboardPage: React.FC = () => {
  ...
  const base = useQuery({
    queryKey: ['base', id],
    queryFn: async () => {
      const { data, error } = await supabase.from('bases').select('id,name').eq('id', id as string).single()
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-neutral-500"><Link to="/dashboard" className="hover:underline">All entities</Link> / Base</div>
