 ...
 *
 * Manages the PowerSync database instance following best practices.
...
class PowerSyncSystem {
   ...
   * PowerSync handles retries and connection management automatically
   */
  public async connect(): Promise<void> {
    ...
    // Wait for seed to complete to avoid nested transaction conflicts
    ...
    // PowerSync handles connection retries automatically
  ...
  private async seedBibleStructureIfNeeded(): Promise<void> {
    ...
    if (
      ...
      logger.info('PowerSync: Bible structure already present, skipping seed');
      return;
    }

    const seedStart = Date.now();
    logger.info('PowerSync: Seeding bible structure via attached seed DB...');
    ...
    try {
      ...
      logger.info('PowerSync: Seed completed', {
        attachedFrom,
        elapsedMs,
      });
      if (attached) {
  ...
  private async seedBibleStructureIfNeededInBackground(): Promise<void> {
    try {
      ...
      logger.warn('PowerSync: background seed failed (non-fatal):', e);
    }
  }
