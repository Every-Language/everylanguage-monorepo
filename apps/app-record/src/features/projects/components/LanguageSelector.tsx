...
import { useSearchLanguages } from '@/features/languages/hooks';
 ...
 *
 * Reusable component for selecting languages with search functionality.
 */
export const LanguageSelector: React.FC<LanguageSelectorProps> = ({
  ...
  const {
    ...
    error,
    refetch,
  } = useSearchLanguages(searchQuery, true);

  const renderItem = useCallback(
    ({ item }: { item: NonNullable<typeof searchResults>[0] }) => (
      <LanguageCard language={item} onPress={onLanguageSelect} />
    ),
    [onLanguageSelect]
  );
