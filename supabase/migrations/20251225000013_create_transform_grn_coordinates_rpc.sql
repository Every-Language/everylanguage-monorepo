-- 20251225000013_create_transform_grn_coordinates_rpc.sql
-- Transform GRN coordinates cache into language_entities_regions
-- Matches GRN numbers to language_entity_ids and country names to region_ids
...
BEGIN
  ...
  WITH
    ...
    -- Prepare upserts (only where both matches found)
    to_upsert AS (
      SELECT
...
comment ON function transform_grn_coordinates_cache_to_language_entities_regions () IS 'Transforms GRN coordinates cache into language_entities_regions. Matches GRN numbers to language_entity_ids and country names to region_ids. Only updates rows where location_source IS NULL or location_source = ''GRN'' to preserve manual entries.';
