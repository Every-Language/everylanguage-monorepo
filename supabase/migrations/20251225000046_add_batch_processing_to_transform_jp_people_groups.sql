-- Add batch processing to transform_jp_people_groups_cache to prevent timeouts
-- Processes entries in batches of 1000 (configurable via batch_size parameter)
CREATE OR REPLACE FUNCTION transform_jp_people_groups_cache (
...
BEGIN
  -- Validate and set batch size (default 1000, max 5000 to prevent memory issues)
  v_batch_limit := LEAST(GREATEST(COALESCE(batch_size, 1000), 1), 5000);
  
  -- Process cache entries in batches
  -- Skip entries that have already been processed (exist in people_groups_regions)
  FOR v_cache_row IN
    SELECT c.*
    FROM jp_people_groups_cache c
    WHERE NOT EXISTS (
      SELECT 1
      FROM people_groups_regions pgr
      WHERE pgr.people_id3_rog3 = c.people_id3_rog3
        AND pgr.deleted_at IS NULL
    )
    ORDER BY c.people_id3_rog3
    LIMIT v_batch_limit
    OFFSET start_offset
  LOOP
    BEGIN
