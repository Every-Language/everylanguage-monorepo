-- Fix get_global_sessions_heatmap RPC function security
-- Change from SECURITY INVOKER to SECURITY DEFINER to bypass RLS policies
-- This allows the function to read all sessions for analytics aggregation
-- even when called by users who can only see their own sessions
...
comment ON function public.get_global_sessions_heatmap (
  ...
  DOUBLE PRECISION,
  INTEGER
) IS 'Optimized PostGIS RPC function for fetching global sessions heatmap data. Performs spatial filtering (bbox) using spatial index, time filtering, and aggregation at database level. Returns grid-aggregated session data with intensity, language distribution, and age normalization. Uses SECURITY DEFINER to bypass RLS policies for analytics aggregation.';
