-- 20251226000054_fix_grn_matching_simplified.sql
-- Simplified GRN matching: ISO-based matching to prevent merging different languages with same names
-- Key insight: GRN children inherit parent's ISO code, so ISO + name uniquely identifies languages
BEGIN;
...
-- Create the transform function with ISO-based matching logic
...
BEGIN
  ...
  -- Step 1: Match/create parents - find existing JP sources with matching ISO, or create new
  -- GRN parents should link to same entity as JP if they share an ISO code
  WITH grn_parents_with_jp AS (
    -- First, try to match GRN parents to existing JP sources by ISO code
    INSERT INTO language_entity_sources (language_entity_id, source, version, is_external, external_id, external_id_type)
    SELECT DISTINCT ON (grn.grn_language_id)
      les_jp.language_entity_id,
      'grn',
      NULL,
      TRUE,
    ...
    JOIN jp_language_cache jp ON jp.iso639_3 = grn.iso639_3
    JOIN language_entity_sources les_jp ON les_jp.external_id = jp.iso639_3
      AND les_jp.external_id_type = 'iso639_3'
      AND les_jp.deleted_at IS NULL
    ...
    ORDER BY grn.grn_language_id, les_jp.language_entity_id
    RETURNING id
  )
  SELECT COUNT(*) INTO v_grn_matched FROM grn_parents_with_jp;
  
  -- Step 2: Create new parent entities for GRN parents not matched to JP
  WITH new_parents AS (
    INSERT INTO language_entities (name, level, parent_id)
    SELECT DISTINCT ON (grn.grn_language_id)
      grn.language_name,
...
comment ON function transform_language_caches_to_entities () IS 'ISO-based transform to prevent merging different languages with same names. JP: matches by name. GRN Phase 1: parents match to JP by ISO or create new. GRN Phase 2: children match by parent GRN ID + ISO verification + name. Prevents "Kalagan: East" (ISO kge) from matching "Kalagan: East" (ISO kqe).';
