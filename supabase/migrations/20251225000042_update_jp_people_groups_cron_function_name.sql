-- Update cron jobs to use renamed functions:
-- 1. sync-jp-people-groups (was sync-jp-people-groups-cache) - Edge Function
-- 2. transform_jp_people_groups_cache (was sync_jp_people_groups_canonical) - RPC Function
BEGIN;
...
-- Update cache sync cron to use new function name sync-jp-people-groups
SELECT
  ensure_http_cron_job_with_body (
    'sync_jp_people_groups_cache_weekly',
    '0 3 * * 0',
    '/functions/v1/sync-jp-people-groups',
    '{"maxPages": 50}'::JSONB
  );


-- Update transform function cron to use renamed function transform_jp_people_groups_cache
DO $$
DECLARE
  v_job_id BIGINT;
BEGIN
  ...
  IF v_job_id IS NULL THEN
    ...
    PERFORM cron.schedule(
      ...
      '0 4 * * 0',
      'SELECT transform_jp_people_groups_cache();'
    );
  ELSE
    -- Update existing job
    PERFORM cron.alter_job(
      job_id := v_job_id,
      schedule := '0 4 * * 0',
      command := 'SELECT transform_jp_people_groups_cache();'
    );
  END IF;
END $$;
