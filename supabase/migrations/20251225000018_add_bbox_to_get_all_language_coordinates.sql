-- 20251225000018_add_bbox_to_get_all_language_coordinates.sql
-- Add bbox filtering to get_all_language_coordinates function for viewport-based fetching
-- This enables efficient spatial filtering using PostGIS spatial index
BEGIN;
...
-- Recreate function with bbox parameters for viewport-based fetching
...
comment ON function get_all_language_coordinates (
  ...
  INTEGER,
  TEXT
) IS 'Returns language coordinate points filtered by bounding box (viewport), optionally filtered by location_source, including bible translation status. Uses PostGIS spatial index for efficient bbox filtering.';
