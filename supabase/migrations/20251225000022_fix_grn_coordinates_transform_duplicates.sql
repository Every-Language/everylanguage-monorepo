  ...
  WITH
    ...
    to_upsert AS (
      SELECT DISTINCT
        ...
        md.region_id,
        -- Use the first location encountered for each (language_entity_id, region_id) pair
        (SELECT md2.location FROM matched_data md2 
         WHERE md2.language_entity_id = md.language_entity_id 
           AND md2.region_id = md.region_id 
           AND md2.location IS NOT NULL 
         LIMIT 1) AS location
      FROM
        matched_data md
      WHERE
