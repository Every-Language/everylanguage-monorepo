-- 20251226000056_fix_grn_match_all_iso_sources.sql
-- Fix GRN matching to check against ALL existing entities with matching ISO codes,
-- not just those with JP sources. This prevents creating duplicates of entities
-- that already exist from SIL seed data.
BEGIN;
...
-- Create the transform function with corrected ISO-based matching logic
...
BEGIN
  ...
  -- This includes entities from SIL (seed data), JP, or any other source
  INSERT INTO language_entity_sources (language_entity_id, source, version, is_external, external_id, external_id_type)
  ...
  -- Match to ANY existing source with this ISO code (SIL, JP, etc.)
  JOIN language_entity_sources les_iso ON les_iso.external_id = grn.iso639_3
    AND les_iso.external_id_type = 'iso639_3'
...
comment ON function transform_language_caches_to_entities () IS 'ISO-based transform to prevent duplicates. JP: matches by name. GRN Phase 1: parents match to ANY existing entity with matching ISO (SIL, JP, etc.) or create new. GRN Phase 2: children match by parent GRN ID + ISO verification + name. Prevents duplicates like "Abipon" (already exists from SIL seed).';
