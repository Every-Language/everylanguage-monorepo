-- Create helper functions for project permission resolution
-- Function to resolve project_id from any table
CREATE OR REPLACE FUNCTION public.resolve_project_id (p_table_name TEXT, p_record_id UUID) returns UUID language plpgsql stable AS $$
DECLARE
  v_project_id UUID;
BEGIN
  CASE p_table_name
...
-- Function to check project permissions (direct, base-project, partner-project)
