-- Create a function to get partner org members with their roles
-- This bypasses RLS to allow reading roles table when querying user_roles
-- Similar to get_user_roles but for partner org members
...
BEGIN
  ...
  WHERE ur.context_type = 'partner'
    AND ur.context_id = p_partner_org_id;
END;
$$;
...
comment ON function public.get_partner_org_members IS 'Safely retrieves partner org members with their roles without triggering RLS recursion. Uses SECURITY DEFINER to bypass RLS policies. Users must have partner.read or partner.manage_roles permission for the partner org.';
